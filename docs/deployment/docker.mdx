...
Use `docker-compose.build.yml` when you want to build the production API and UI images from the repository source instead of pulling prebuilt images:

```shell
docker compose -f docker-compose.build.yml build
```

You can override the output image tags if you want to load them into a different registry or attach a custom tag:

```shell
KEEP_FRONTEND_IMAGE=ghcr.io/your-org/keep-ui:latest \
KEEP_BACKEND_IMAGE=ghcr.io/your-org/keep-api:latest \
docker compose -f docker-compose.build.yml build
```
